...
class TrackEventUseCaseTest {
    ...
    @Test
    fun `when use case is invoked then track event in repository`() = runTest {
        // Given
    ...
    @Test
    fun `when use case is invoked with provider key then track event in specific repository provider`() = runTest {
        // Given
        val event = AnalyticsEvent.Custom("test_event")
