...
class AnalyticsDataSourceTest {
    ...
    @Test
    fun `when sendEvent is called then all registered providers track the event`() = runTest {
        // Given
    ...
    @Test
    fun `when sendEvent is called with key then only matching provider tracks the event`() = runTest {
        // Given
        val event = AnalyticsEvent.Custom("test_event")
        dataSource.addProvider(provider1)
        dataSource.addProvider(provider2)
    ...
    @Test
    fun `when provider is removed then it no longer tracks events`() = runTest {
        // Given
        val event = AnalyticsEvent.Custom("test_event")
        dataSource.addProvider(provider1)
    ...
    @Test
    fun `when adding provider with same key then it is not duplicated`() = runTest {
        // Given
        val event = AnalyticsEvent.Custom("test_event")
        dataSource.addProvider(provider1)
        dataSource.addProvider(provider1)
